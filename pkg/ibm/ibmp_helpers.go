...
// listInstances returns all of the Power Systems Virtual Server instances on ibmp's cloud instance.
func (ibmp IBMPowerDynamicConfig) listInstances(ctx context.Context, service *core.BaseService) (models.PVMInstances, error) {
...
// parsePowerPcCRN returns the service instance of ibmp's Cloud Resource Name.
...
// launchInstance returns the instance ID of the Power Systems Virtual Server Instance created with an HTTP
// request on ibmp's cloud instance.
func (ibmp IBMPowerDynamicConfig) launchInstance(ctx context.Context, service *core.BaseService, name string) (cloud.InstanceIdentifier, error) {
	log := logr.FromContextOrDiscard(ctx)
...
func (ibmp IBMPowerDynamicConfig) getInstance(ctx context.Context, service *core.BaseService, pvmId string) (*models.PVMInstance, error) {
	...
	_, err = requestBuilder.ResolveRequestURL(ibmp.Url, `/pcloud/v1/cloud-instances/{cloud}/pvm-instances/{pvm_instance_id}`, pathParamsMap)
	if err != nil {
...
func (ibmp IBMPowerDynamicConfig) resizeInstanceVolume(ctx context.Context, service *core.BaseService, id *string) {
	...
	go func() {
		...
		for {
			...
			if err != nil {
				log.Error(err, "failed to get Power Systems VM instance for resize, retrying in %s", sleepTime)
				continue
			}
			...
			log.Info("Resizing instance volume", "instance", *id, "volumeID", instance.VolumeIDs[0], "size", ibmp.Disk)
			err = ibmp.updateVolume(localCtx, service, instance.VolumeIDs[0])
			if err != nil {
				continue
			}
			log.Info("Successfully resized instance volume", "instance", *id, "volumeID", instance.VolumeIDs[0], "size", ibmp.Disk)
			return
		}
	}()
}
...
func (ibmp IBMPowerDynamicConfig) updateVolume(ctx context.Context, service *core.BaseService, volumeId string) error {
	...
	_, err = requestBuilder.ResolveRequestURL(ibmp.Url, `/pcloud/v1/cloud-instances/{cloud}/volumes/{volume}`, pathParamsMap)
	if err != nil {
		return fmt.Errorf("failed to encode the request with parameters: %w", err)
	}
