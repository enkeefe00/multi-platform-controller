...
func (r DynamicResolver) Allocate(taskRun *ReconcileTaskRun, ctx context.Context, tr *v1.TaskRun, secretName string) (reconcile.Result, error) {
	...
	if tr.Annotations[CloudInstanceId] != "" {
		...
		if err != nil { // A permanent error occurred when fetching the IP address for the VM
			...
			//An error occurred while getting the VM state or it's in a bad state
			if state == "FAILED" || err != nil {
				...
				} else { //VM is in a failed state and the task needs to be requeued
					log.Info("VM instance is in a failed state; will attempt to terminate, unassign from task, and re-queue task")
				}
				//Try to delete the instance and unassign it from the TaskRun
				terr := r.CloudProvider.TerminateInstance(taskRun.client, ctx, cloud.InstanceIdentifier(tr.Annotations[CloudInstanceId]))
				if terr != nil {
				...
				unassignErr := r.removeInstanceFromTask(taskRun, ctx, tr)
				if unassignErr != nil {
					msg := "failed to unassign instance from task after instance state retrieval failure"
					log.Error(unassignErr, msg)
				} else if state == "FAILED" { //Only try to re-queue task if unassignment is a success & VM is in a failed state
					return reconcile.Result{RequeueAfter: time.Minute}, nil
				}
				return reconcile.Result{}, err
			}
			//Most likely waiting for the instance to come up, so just re-queue and wait
